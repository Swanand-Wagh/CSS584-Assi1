...
export const IntensityMethod = () => {
  ...
  const intensityMethodFilterAlgo = async () => {
    let imageSizes = [];
    let histograms = [];

    let minDistances = [];
    for (let i = 0; i < 100; i++) {
      minDistances[i] = { distanceFrom: i, distanceWith: -1, distance: 9999 };
    }
  ...
  return { intensityMethodFilterAlgo };
};
